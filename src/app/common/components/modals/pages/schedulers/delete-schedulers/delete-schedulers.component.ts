...
@Component({
  selector: 'app-delete-schedulers',
  templateUrl: './delete-schedulers.component.html',
  styleUrls: ['./delete-schedulers.component.css']
})
export class DeleteSchedulersComponent implements OnInit {
  ...
  onSubmit() {
    this.automationsService.deleteScheduler(this.id?.value).then((response) => {
      if (!response.error) {
        this.modal.close();
        this.loader.refresh();
      }
